...
try {
    const result = spawnSync('./mvnw', ['-q', 'spring-boot:run', `"-Dspring-boot.run.arguments=${commandArgs} --server.port=0"`], { stdio: 'inherit', env: { ...process.env, SKIP_FRONTEND: 'true' }, shell: true });
    if (result.error) {
        throw result.error;
    }
