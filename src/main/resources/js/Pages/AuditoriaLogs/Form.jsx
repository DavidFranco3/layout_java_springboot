...
export default function Form(props) {
    const { mode, routeBase, auditoriaLog } = props;
    const isEdit = mode === "update";

    const { data, setData, post, put, processing } = useForm({
        campo_ejemplo: isEdit ? auditoriaLog?.campo_ejemplo || "" : "",
    });

    const handleSubmit = (e) => {
        e.preventDefault();

        if (isEdit && auditoriaLog?.id) {
            put(`/${routeBase}/${auditoriaLog.id}`);
        } else {
            post(`/${routeBase}`);
        }
    };
        ...
        <Authenticated auth={props.auth} errors={props.errors}>
            <div className="col-lg-12 d-flex justify-content-center">
                <div className="col-lg-12 col-lg-offset-1 mt-2">
                    <ContainerLaravel
                        titulo={isEdit ? "Editar AuditoriaLog" : "Crear AuditoriaLog"}
