        ...
        <>
            ...
            >
                    ...
                    <Link href={route("dashboard")} className={`flex items-center justify-center w-full h-full transition-all duration-500 !no-underline ${sidebarOpen ? 'px-4' : 'px-0'}`}>
                        {configuracion?.logo ? (
                            ...
                            <div className="flex items-center justify-center gap-4">
                                <div className="w-14 h-14 rounded-[20px] bg-white text-primary flex items-center justify-center text-2xl font-black shadow-2xl shadow-black/40 group-hover:scale-110 transition-transform duration-500">
                                    {configuracion?.nombre_comercial?.charAt(0) || 'D'}
                                </div>
                                {sidebarOpen && (
                                    <span className={`text-2xl font-black tracking-tighter transition-all duration-500 ${isSidebarDark ? 'text-white' : 'text-slate-800'}`}>
                ...
                <div className={`px-2 py-6 transition-all duration-500`}>
                    <div className={`relative group p-3 rounded-[24px] overflow-hidden transition-all duration-500 ${sidebarOpen ? (isSidebarDark ? 'bg-white/5' : 'bg-black/5') : 'bg-transparent py-0 px-1'}`}>
                        <div className={`flex items-center ${sidebarOpen ? 'gap-3' : 'flex-col gap-3'}`}>
                            ...
                            <div className={`flex flex-col min-w-0 transition-all duration-700 ${sidebarOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-50 hidden'}`}>
                        ...
                        <ul className="space-y-1">
                            <li>
                                ...
                                >
                                    ...
                                    <span className={`ml-3 text-sm font-bold tracking-tight relative z-10 transition-all duration-500 ${sidebarOpen ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>
                                        Dashboard
                                    </span>
                                </Link>
                            </li>
