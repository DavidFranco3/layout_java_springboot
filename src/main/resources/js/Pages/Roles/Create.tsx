...
export default function Create({ cerrarModal, permisos, modulos, onRefresh }: RoleCreateProps) {
    ...
    useEffect(() => {
        if (permisos && permisos.length > 0) {
            ...
            if (primerModulo) setActiveTab(primerModulo);
        }
    }, [permisos]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            Swal.fire({
                icon: "error",
                title: "Error",
                text: err.response?.data?.message || "No se pudo registrar el rol. Revisa los datos.",
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 animate-fade-in p-1">
            <div className="space-y-2">
                ...
                <div className="relative">
                    ...
                    <TextInput
                        ...
                        isError={!!errors.name || !!serverErrors.name}
                        {...register("name", { required: "El nombre del rol es requerido" })}
                    />
                </div>
                <InputError message={errors.name?.message || serverErrors.name} />
            </div>
                    ...
                    <div className="max-h-[300px] overflow-y-auto p-4 space-y-2 custom-scrollbar">
                        {activeTab && permisosPorModulo[activeTab]?.map((permiso) => (
                            <div
                                ...
                                className={`group flex items-center justify-between p-3 rounded-2xl cursor-pointer transition-all border ${selectedPermisos.includes(permiso.id)
                                    ? "bg-primary/5 border-primary/20 shadow-sm"
                                    : "bg-white dark:bg-slate-800/40 border-slate-100 dark:border-slate-800 hover:border-primary/30"
                                    }`}
                            >
                                <div className="flex items-center gap-3">
                                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${selectedPermisos.includes(permiso.id) ? "bg-primary text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-400 group-hover:text-primary"}`}>
                                        <FontAwesomeIcon icon={faCheckCircle} className="text-xs" />
                                    </div>
                                    <div>
                                        <p className={`text-sm font-bold transition-colors ${selectedPermisos.includes(permiso.id) ? "text-primary" : "text-slate-700 dark:text-slate-200"}`}>
                                            {permiso.name}
                                        </p>
                                        <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-medium">Acceso de módulo</p>
                                    </div>
                                </div>
                                <div className={`w-10 h-5 rounded-full relative transition-colors ${selectedPermisos.includes(permiso.id) ? "bg-primary" : "bg-slate-200 dark:bg-slate-700"}`}>
                                    <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all shadow-sm ${selectedPermisos.includes(permiso.id) ? "left-6" : "left-1"}`} />
                                </div>
                            </div>
                        ))}
                    </div>
                </div>
                <InputError message={serverErrors.permisos} />
                    ...
                    <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500">{selectedPermisos.length} permisos habilitados para este rol</p>
                            </div>
                        </div>
                        <span className="text-lg font-black text-primary">{selectedPermisos.length}</span>
                    </div>
                )}
            </div>

            <div className="flex items-center justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                <SecondaryButton type="button" onClick={cerrarModal} className="h-12 px-6 font-bold">
